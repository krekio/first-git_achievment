...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed(action, duration)*1000.0/3600.0, 2)/(height/100.0))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
